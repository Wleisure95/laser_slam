...
typedef Map<double, DoubleArray2D, false> DoubleMap;	//浮点数地图

template <class Cell, class Storage, const bool isClass>
  const Cell  Map<Cell,Storage,isClass>::m_unknown = Cell(-1);

/*
@desc Map构造函数，设置地图大小以及分辨率
@param	mapSizeX	地图大小  地图坐标 整数
@param  mapSizeY    地图大小
@param  delta       地图分辨率
*/
template <class Cell, class Storage, const bool isClass>
...
/*
@desc  Map构成函数，要把世界大小转换成地图大小
@param center		中心
@param worldSizeX	世界大小  物理坐标
@param worldSizeY	世界大小
@param delta		分辨率
*/
template <class Cell, class Storage, const bool isClass>
